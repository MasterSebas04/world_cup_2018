...
for i, event in enumerate(event_data[:-1]):
    ...
    if passer_id is not None and team_id is not None and passer_id in player_by_id:
        p = player_by_id[passer_id]
        first_name = unicode_conversion(p.get("firstName", ""))
        last_name = unicode_conversion(p.get("lastName", ""))
        passer_name = f"{first_name} {last_name}".strip()
    ...
    if receiver_id is not None and receiver_team_id is not None and receiver_id in player_by_id:
        p = player_by_id[receiver_id]
        first_name = unicode_conversion(p.get("firstName", ""))
        last_name = unicode_conversion(p.get("lastName", ""))
        receiver_name = f"{first_name} {last_name}".strip()
...
for i, event in enumerate(event_data[:-1]):
    ...
    # Use full name (firstName + lastName) to uniquely identify players
    passer_p = player_by_id[passer]
    passer_first = unicode_conversion(passer_p.get("firstName", ""))
    passer_last = unicode_conversion(passer_p.get("lastName", ""))
    passer_name = f"{passer_first} {passer_last}".strip()
    
    receiver_p = player_by_id[receiver]
    receiver_first = unicode_conversion(receiver_p.get("firstName", ""))
    receiver_last = unicode_conversion(receiver_p.get("lastName", ""))
    receiver_name = f"{receiver_first} {receiver_last}".strip()
    
    key = tuple(sorted([passer_name, receiver_name]))

    result_tally[key] = result_tally.get(key, 0) + 1

# print("Player to Team", player_to_team)
# print("Result Tally", result_tally)
...
# Get unique teams from nodes in the graph (only needed if you want to count teams)
nodes_in_graph = list(G.nodes())
teams_in_graph = set()
for node in nodes_in_graph:
    # Checks if str is in key
    if node in player_to_team:
        teams_in_graph.add(player_to_team[node])

# Add team info as node attributes for Gephi (Gephi will handle coloring!)
for node in nodes_in_graph:
    ...
    if team_id:
        ...
        G.nodes[node]["team_id"] = None
        G.nodes[node]["team_name"] = "Unknown"
